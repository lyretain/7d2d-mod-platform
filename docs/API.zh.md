...
如果 Mod 版本声明了兼容游戏版本，而 Pack 的游戏版本不在范围内，后台会拒绝创建。当前客户端版本串 `3.1.0` 与服务端版本串 `3.10.14` 会按同一游戏 Build 处理。`gameVersionRange` 为 `major`（泛版本）时，声明 `3.0` 可打进 `3.10.14` 的 Pack，但不能打进 `4.0`。未勾选时仍按精确匹配。
