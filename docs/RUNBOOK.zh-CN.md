...
1. 新容器先通过 `/health/live` 和 `/health/ready`（数据库、对象存储、签名服务均 ok）。
2. 多实例必须使用 `DATABASE_URL` 和共享对象存储，禁止多实例写同一 JSON 文件。
3. 反向代理切流量；旧实例排空后退出。
4. 失败则把代理指回旧实例，数据库按上一节回滚。
...
本仓库运行时零 npm 依赖。上线前仍应：

- 确认镜像基于官方 Node 22，并安装系统安全更新
