...
test('content slots attach overlays that publish into the release', async (t) => {
  ...
  const dressed = createStoredZip({ 'Avatars/hero.avatar3d': 'dressed', 'Avatars/Preview.png': 'img', 'Avatars/README.md': '# hero' });
  const frameworkSha = sha256(framework);
  const heroSha = sha256(hero);
  const villainSha = sha256(villain);
  const dressedSha = sha256(dressed);
  ...
  await jsonRequest(`${base}/api/v1/artifacts/${dressedSha}`, { method: 'PUT', headers: { ...admin, 'content-type': 'application/zip' }, body: dressed });
  ...
  const third = await jsonRequest(`${base}/api/v1/mods/z-custom-avatars/slots/avatars/contents`, { method: 'POST', headers: admin, body: JSON.stringify({ artifactSha: dressedSha, name: 'Dressed' }) });
  assert.equal(first.name, 'Hero');
  assert.equal(second.slotId, 'avatars');
  assert.equal(third.previewPath, 'Avatars/Preview.png');
  assert.equal(third.readmePath, 'Avatars/README.md');
  const patched = await jsonRequest(`${base}/api/v1/contents/${first.id}`, { method: 'PATCH', headers: admin, body: JSON.stringify({ name: 'Hero Plus', description: 'Updated', r18: true }) });
  assert.equal(patched.name, 'Hero Plus');
  assert.equal(patched.description, 'Updated');
  assert.equal(patched.r18, true);
  const listed = await jsonRequest(`${base}/api/v1/mods/z-custom-avatars`, { headers: { authorization: admin.authorization } });
  assert.equal(listed.contentCounts.avatars, 3);
  assert.equal(listed.contents.length, 3);
  ...
  assert.equal(market.contents.length, 3);
  ...
  assert.equal(latest.mods[0].overlays[0].name, 'Hero Plus');
...
test('R18 mods and contents stay redacted until adult confirmation', async (t) => {
  ...
  const adultAudit = await jsonRequest(`${base}/api/v1/admin/audit?action=${encodeURIComponent('auth.adult_confirm')}`, { headers });
  assert.equal(adultAudit.audit.length, 1);
  assert.equal(adultAudit.audit[0].action, 'auth.adult_confirm');
  assert.ok(!JSON.stringify(adultAudit.audit[0]).includes('1990'));
