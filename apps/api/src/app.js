...
:root{--bg:#111;--panel:#1a1a1a;--card:#222;--line:#3a3a3a;--text:#eee;--muted:#9a9a9a;--accent:#ffb000;--danger:#9b2f24;--ok:#2f6f4e}
*{box-sizing:border-box}html,body{margin:0;background:var(--bg);color:var(--text);font:15px/1.45 system-ui,"Microsoft YaHei",sans-serif}
header{position:sticky;top:0;z-index:5;background:#161616;border-bottom:1px solid var(--line);padding:12px 20px}
.brand{display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap}
h1{margin:0;color:var(--accent);font-size:20px}h2{margin:0 0 12px;font-size:16px}p.hint{margin:0 0 12px;color:var(--muted);font-size:13px}
.status{margin:0;padding:6px 10px;background:#252525;border-radius:6px;font-size:13px}
nav{display:flex;gap:8px;flex-wrap:wrap;margin-top:12px}
nav button{width:auto;margin:0;padding:7px 12px;background:#2a2a2a;border:1px solid var(--line)}
nav button.active{background:var(--danger);border-color:var(--danger)}
.shell{display:grid;grid-template-columns:minmax(0,1fr) minmax(320px,420px);gap:16px;max-width:1280px;margin:0 auto;padding:16px 20px 32px}
.stack{display:flex;flex-direction:column;gap:14px}
section{display:none;background:var(--panel);border:1px solid var(--line);border-radius:10px;padding:16px}
section.on{display:block}
label.field{display:block;margin:8px 0 4px;color:var(--muted);font-size:12px}
input,select,button,textarea{width:100%;padding:9px 10px;margin:0 0 8px;background:#2b2b2b;color:#fff;border:1px solid #555;border-radius:6px}
button{background:var(--danger);cursor:pointer;border:0;font-weight:600}
button.secondary{background:#3a3a3a}button.ok{background:var(--ok)}button.wide{margin-top:4px}
.actions{display:flex;gap:8px;flex-wrap:wrap}.actions button{width:auto;flex:1;min-width:120px;margin:0}
.row{display:grid;grid-template-columns:1fr 1fr;gap:10px}.three{display:grid;grid-template-columns:1fr 1fr 1fr;gap:10px}
.check{display:flex;align-items:center;gap:8px;margin:8px 0 12px;color:var(--muted)}
.check input{width:auto;margin:0}
.output{position:sticky;top:84px;align-self:start;background:var(--card);border:1px solid var(--line);border-radius:10px;padding:14px;min-height:280px}
pre{white-space:pre-wrap;background:#0c0c0c;padding:12px;max-height:calc(100vh - 180px);overflow:auto;margin:8px 0 0;border-radius:6px;font-size:12px}
#inviteResult,#uploadResult{max-height:180px}
@media(max-width:900px){.shell{grid-template-columns:1fr;padding:12px}.output{position:static}.row,.three{grid-template-columns:1fr}nav button,.actions button{flex:1}}
</style></head><body>
<header>
  <div class="brand"><h1>七日杀 Mod 平台</h1><p class="status" id="authStatus">尚未登录</p></div>
  <nav>
    <button class="active" data-tab="account" onclick="showTab('account')">账户</button>
    <button data-tab="mods" onclick="showTab('mods')">Mod</button>
...
<div class="shell">
  <div class="stack">
    <section id="tab-account" class="on">
      <h2>登录</h2>
      <div class="row"><div><label class="field">用户名</label><input id="loginUser" autocomplete="username" placeholder="用户名"></div><div><label class="field">密码</label><input id="loginPassword" type="password" autocomplete="current-password" placeholder="密码"></div></div>
      <div class="actions"><button onclick="login()">登录</button><button class="secondary" onclick="logout()">退出登录</button></div>
      <h2 style="margin-top:22px">邀请码注册</h2>
      <div class="three"><div><label class="field">用户名</label><input id="registerUser" autocomplete="username" placeholder="3-32位"></div><div><label class="field">密码</label><input id="registerPassword" type="password" autocomplete="new-password" placeholder="至少10位"></div><div><label class="field">邀请码</label><input id="registerInvite" placeholder="inv_..."></div></div>
      <button class="wide" onclick="register()">注册</button>
      <h2 style="margin-top:22px">首次部署 / 邀请管理</h2>
      <label class="field">ADMIN_TOKEN（仅首次部署）</label>
      <input id="bootstrapToken" type="password" placeholder="首次部署可在此填写 ADMIN_TOKEN">
      <div class="three"><div><label class="field">角色</label><select id="inviteRole"><option value="admin">管理员</option><option value="viewer">只读用户</option></select></div><div><label class="field">可用次数</label><input id="inviteUses" type="number" min="1" max="100" value="1"></div><div><label class="field">有效小时</label><input id="inviteHours" type="number" min="1" max="8760" value="168"></div></div>
      <button class="wide" onclick="createInvite()">创建邀请码</button>
      <pre id="inviteResult">邀请码只显示一次，请安全发送给受邀用户。</pre>
    </section>
    <section id="tab-mods">
      <h2>上传 Mod ZIP</h2>
      <input id="artifact" type="file" accept=".zip">
      <button class="wide" onclick="upload()">上传并计算哈希</button>
      <pre id="uploadResult">上传结果会显示在这里，并自动填入 SHA-256。</pre>
      <h2 style="margin-top:22px">注册 Mod 版本</h2>
      <div class="row"><div><label class="field">Mod ID</label><input id="modId" placeholder="example-vehicles"></div><div><label class="field">显示名称</label><input id="modName" placeholder="Example Vehicles"></div></div>
      <div class="row"><div><label class="field">版本</label><input id="modVersion" placeholder="1.0.0"></div><div><label class="field">文件 SHA-256</label><input id="artifactSha" placeholder="上传后自动填写"></div></div>
      <label class="field">游戏版本</label><input id="gameVersions" placeholder="逗号分隔，例如 3.10.14">
      <label class="field">ZIP 顶层目录</label><input id="installRoots" placeholder="逗号分隔，例如 ExampleVehicles">
      <label class="check"><input id="containsDll" type="checkbox"> 包含客户端 DLL / 需要重启</label>
      <div class="actions"><button onclick="registerMod()">注册 Mod 版本</button><button class="secondary" onclick="loadMods()">Mod 列表</button></div>
    </section>
    <section id="tab-packs">
      <h2>创建并发布 ModPack</h2>
      <div class="row"><div><label class="field">Pack ID</label><input id="packId" placeholder="production-pack"></div><div><label class="field">名称</label><input id="packName" placeholder="正式服 Pack"></div></div>
      <div class="row"><div><label class="field">游戏版本</label><input id="packGame" placeholder="3.10.14"></div><div><label class="field">条目</label><input id="packEntries" placeholder="mod@version,mod2@version"></div></div>
      <button class="wide" onclick="publishPack()">创建并发布</button>
      <h2 style="margin-top:22px">发布、吊销与回滚</h2>
      <p class="hint">回滚可能影响已有存档，请确认后再执行。</p>
      <div class="row"><div><label class="field">Pack ID</label><input id="releasePackId" placeholder="默认使用上方 Pack ID"></div><div><label class="field">Release ID</label><input id="releaseId" placeholder="rel_..."></div></div>
      <label class="field">原因</label><input id="releaseReason" placeholder="发布 / 吊销 / 回滚原因">
      <div class="actions"><button class="secondary" onclick="listReleases()">列出 Release</button><button class="secondary" onclick="loadPacks()">Pack 列表</button></div>
      <div class="actions" style="margin-top:8px"><button onclick="revokeRelease()">吊销 Release</button><button onclick="rollbackRelease()">回滚到该 Release</button></div>
    </section>
    <section id="tab-servers">
      <h2>服务器绑定与紧急停发</h2>
      <div class="row"><div><label class="field">Server ID</label><input id="serverId" placeholder="srv_..."></div><div><label class="field">新的 Pack ID</label><input id="serverPackId" placeholder="production-pack"></div></div>
      <div class="row"><div><label class="field">公开地址</label><input id="serverAddress" placeholder="host:port"></div><div><label class="field">停发原因</label><input id="pauseReason" placeholder="可选"></div></div>
      <div class="actions"><button onclick="updateServer()">更新服务器绑定</button><button class="secondary" onclick="loadServers()">服务器状态</button></div>
      <div class="actions" style="margin-top:8px"><button onclick="pauseDistribution(true)">紧急停止分发</button><button class="ok" onclick="pauseDistribution(false)">恢复分发</button></div>
    </section>
    <section id="tab-ops">
      <h2>目录与运维</h2>
      <div class="actions"><button class="secondary" onclick="loadReviews()">审核队列</button><button class="secondary" onclick="loadMatrix()">兼容性矩阵</button><button class="secondary" onclick="loadStats()">下载统计</button><button class="secondary" onclick="loadUsers()">用户列表</button><button class="secondary" onclick="loadSessions()">登录会话</button><button class="secondary" onclick="loadState()">平台状态</button></div>
      <div class="row" style="margin-top:12px"><div><label class="field">审计 action</label><input id="auditAction" placeholder="例如 release.publish"></div><div><label class="field">审核 SHA-256</label><input id="reviewSha" placeholder="批准许可证时使用"></div></div>
      <div class="actions"><button onclick="loadAudit()">查询审计</button><button class="ok" onclick="approveReview()">确认许可证并批准</button></div>
    </section>
  </div>
  <aside class="output"><h2>结果</h2><p class="hint">接口返回显示在这里，无需滚回页面底部。</p><pre id="out">尚未读取</pre></aside>
</div>
<script>
function showTab(name){document.querySelectorAll('section').forEach(function(el){el.classList.toggle('on',el.id==='tab-'+name)});document.querySelectorAll('nav button').forEach(function(el){el.classList.toggle('active',el.getAttribute('data-tab')===name)});if(window.innerWidth<900)document.querySelector('.output').scrollIntoView({block:'nearest'})}
